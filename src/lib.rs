...
pub fn render(ipv4: bool, ipv6: bool) -> String {
    ...
    ret.push_str_line("# This file is generated by https://github.com/Zxilly/OnedriveHostsGenerator");

    let now = Local::now();

    ret.push_str_line(&format!("# Generate time: {}", now.format("%Y-%m-%d %H:%M:%S")));
    ...
    if ipv4 {
        ...
        for (domain, ip) in v4_ips.into_iter() {
            ret.push_str_line(&format!("{:w1$} {:>w2$}", ip, domain, w1 = max_v4_ip_len, w2 = max_v4_domain_len));
        }
    }
    ...
    if ipv6 {
        ...
        for (domain, ip) in v6_ips.into_iter() {
            ret.push_str_line(&format!("{:w1$} {:>w2$}", ip, domain, w1 = max_v6_ip_len, w2 = max_v6_domain_len));
        }
    }
